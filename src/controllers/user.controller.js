...
const refreshAccessToken = asyncHandler(async (req, res) => {
   ...
   if(!incomingRefreshToken) {
    throw new ApiError(401, "unathourised acess")
   }
...
const getCurrentUser = asyncHandler(async(req, res)=>{
    return res
    .status(200)
    .json(new ApiResponse(200,req.user,"Current user fetched successfullu"))
})
...
const updateAccountDetails = asyncHandler( async(req, res) => {
    ...
    if(!fullName || !email) {
        throw new ApiError(400,"All fielfd are required")
    }
...
const getUserChannelProfile = asyncHandler( async(req, res) => {
  ...
  if(!channel?.length) {
    throw new ApiError(400,"channel doenot exists ")
  }

  return res
  .status(200)
  .json(
    new ApiResponse(200,channel[0],"User details fecthced successfully")
  )
...
const getWatchHistory = asyncHandler(async (req, res) => {
    ...
    .json(
        new ApiResponse(
            200,
            user[0].watchHistory,
            "watchHistory fecthed succesfully"
            )
    )
})
